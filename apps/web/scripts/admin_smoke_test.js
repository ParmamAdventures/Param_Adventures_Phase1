...
async function login(email, password='Password123!'){
  return await http('/auth/login', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ email, password }) });
}

async function register(email, password='Password123!', name='Smoke Test'){
  return await http('/auth/register', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ email, password, name }) });
}
...
(async ()=>{
  try{
    ...
    console.log('  /admin/users status', r1.status, r1.body && typeof r1.body === 'object' ? JSON.stringify(r1.body) : r1.body);
    ...
    if(normal.status !== 200){ console.log('  Normal user login failed:', normal.status, normal.body); }
    const normalToken = normal.body?.accessToken;
    const r2 = await callWithToken('/admin/users', normalToken);
    console.log('  /admin/users status', r2.status, r2.body && typeof r2.body === 'object' ? JSON.stringify(r2.body) : r2.body);

    console.log('\nTest 3 — Super admin (expect 200)');
    const superLogin = await login('test+1766053638.10583@example.com');
    if(superLogin.status !== 200){ console.log('  Super admin login failed:', superLogin.status, superLogin.body); process.exit(1); }
    const superToken = superLogin.body?.accessToken;
    const ru = await callWithToken('/admin/users', superToken);
    console.log('  /admin/users status', ru.status, Array.isArray(ru.body) ? `users=${ru.body.length}` : JSON.stringify(ru.body));
    const rr = await callWithToken('/admin/roles', superToken);
    console.log('  /admin/roles status', rr.status, Array.isArray(rr.body) ? `roles=${rr.body.length}` : JSON.stringify(rr.body));
