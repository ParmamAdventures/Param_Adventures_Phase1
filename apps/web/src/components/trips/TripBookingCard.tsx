...
export default function TripBookingCard({ trip }: Props) {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      showToast("Booking request sent! You can now proceed to payment.", "success");
    } catch (err: any) {
  ...
  return (
    <Card className="sticky top-24 space-y-6 bg-[var(--card)] border shadow-xl">
      ...
      <div className="space-y-4">
        {!bookingId ? (
          ...
          >
            {disabled ? "Not available" : loading ? "Requesting..." : "Join Trip"}
          </Button>
        ) : (
          <div className="space-y-4">
