...
import { app } from "../app";
import { handleDisputeCreated, handleDisputeLost, handleDisputeWon } from "../controllers/paymentEvents";
...
async function run() {
    try {
        ...
        const admin = await prisma.user.upsert({
            ...
            create: { email: 'admin-dispute@test.com', password: 'pw', name: 'Admin', roles: { create: { role: { create: { name: 'admin' } } } } },
            update: {}
        });
        
        const trip = await prisma.trip.create({
            data: { title: "Dispute Trip", slug: `disp-trip-${Date.now()}`, price: 2000, createdById: admin.id, status: "PUBLISHED", description: "Test", itinerary: {}, difficulty: "EASY", location: "Loc", durationDays: 1 }
        });
        const booking = await prisma.booking.create({ data: { userId: admin.id, tripId: trip.id, totalPrice: 2000, status: "CONFIRMED", startDate: new Date(), paymentStatus: "PAID" } });
        
        const payment = await prisma.payment.create({
            data: { bookingId: booking.id, provider: "razorpay", providerOrderId: "ord_disp_" + Date.now(), providerPaymentId: "pay_disp_" + Date.now(), amount: 200000, status: "CAPTURED" }
        });
