...
async function run() {
    try {
        ...
        const user = await prisma.user.create({
            data: { email: `invoice-feat008-${Date.now()}@test.com`, password: "password", name: "Invoice Tester" }
        });
        ...
        if (res.status === 200 && res.header['content-type'] === 'application/pdf') {
            ...
            if (res.body.length > 1000) { // Empty PDF is small, actual one should be larger
