...
| :---------------------------- | :--------------------------------------------------------- | :------------------------------------------------- |
| **B1. Concurrent Bookings**   | Simulating 2 users booking the _last_ slot simultaneously. | **Inventory**: Prevents overbooking.               |
...
describe("Critical Backend Paths", () => {
  ...
  it("should prevent overbooking when capacity is 1", async () => {
    ...
    const successes = [res1.status, res2.status].filter(
      (s) => s === 201
    ).length;
